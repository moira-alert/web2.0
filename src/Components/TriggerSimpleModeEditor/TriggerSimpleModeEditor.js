...
import type { TriggerType } from "../../Domain/Trigger";
import { Statuses } from "../../Domain/Status";
// import { ValidationWrapperV1, tooltip, type ValidationInfo } from "react-ui-validations";
import { ColumnStack, RowStack, Fit, Fixed } from "@skbkontur/react-stack-layout";
...
export default function TriggerSimpleModeEditor(props: Props): React.Node {
    const { data, onTypeChange, onValueChange } = props;
    const { trigger_type, warn_value, error_value } = data;
    return (
        <ColumnStack block gap={2}>
            <Fit>
                <Radio
                    checked={trigger_type === TriggerTypes.RISING}
                    value={TriggerTypes.RISING}
                    onChange={(evt, value) => {
                        onTypeChange(value);
                    }}>
                        ...
                        <ColumnStack block gap={2}>
                            <Fit>
                                <RowStack block verticalAlign="center" gap={1}>
                                    <Fit>
                                        <StatusIcon status={Statuses.WARN} />
                                    </Fit>
                                    <Fixed width={100}>{"WARN if T1 >="}</Fixed>
                                    <Fit>
                            ...
                            <Fit>
                                <RowStack block verticalAlign="center" gap={1}>
                                    <Fit>
                                        <StatusIcon status={Statuses.ERROR} />
                                    </Fit>
                                    <Fixed width={100}>{"ERROR if T1 >="}</Fixed>
                                    <Fit>
                                        <FormattedNumberInput
                                            width={120}
                                            editFormat={defaultNumberEditFormat}
            ...
            <Fit>
                <Radio
                    checked={trigger_type === TriggerTypes.FALLING}
                    value={TriggerTypes.FALLING}
                    onChange={(evt, value) => {
                        onTypeChange(value);
                    }}>
