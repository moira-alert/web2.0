...
function calculateMaintenanceTime(maintenance: Maintenance): number {
  ...
  return maintenanceTime > 0 ? getUnixTime(addMinutes(getUTCDate(), maintenanceTime)) : maintenanceTime;
}

export async function setMetricMaintenance(moiraApi: IMoiraApi, triggerId: string, metric: string, maintenance: Maintenance) {
  await moiraApi.setMaintenance(triggerId, {
    metrics: { [metric]: calculateMaintenanceTime(maintenance) }
  });
}

export async function setTriggerMaintenance(moiraApi: IMoiraApi, triggerId: string, maintenance: Maintenance) {
